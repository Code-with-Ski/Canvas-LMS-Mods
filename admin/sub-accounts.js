(() => {
  if (/^\/accounts\/[0-9]+\/sub_accounts\??[^\/]*\/?$/.test(window.location.pathname)) {
    chrome.storage.sync.get({
      ...
      updateSubAccountIds(items.adminSubAccountsCanvasId, items.adminSubAccountsSisId);
    });
  }
  ...
  */
  function updateSubAccountIds(isCanvasAccountIdShown, isSisAccountIdShown) {
    if (isCanvasAccountIdShown || isSisAccountIdShown) {
      const topLevelAccount = document.querySelector("div#content > div.account");
      const canvasId = topLevelAccount.querySelector("div.show_account span.id")?.textContent;
      if (canvasId) {
        const accountLinks = topLevelAccount.querySelector("div.show_account span.links");
        ...
        if (isSisAccountIdShown) {
          fetch(`https://${document.location.hostname}/api/v1/accounts/${canvasId}`)
            ...
            .then(data => {
              ...
              accountLinks.insertAdjacentHTML("afterend", `<div style='color: var(--ic-brand-font-color-dark-lightened-15); font-size: 12px; font-size: 0.75rem;' class='ski-sis-account-id'>SIS Account ID: ${sisId}</div>`);
            })
            .catch((error) => {
      ...
      const subAccountDivs = [...topLevelAccount.querySelectorAll("ul.sub_accounts div.account")];
      if (subAccountDivs) {
        let requestNum = 0;
        subAccountDivs.forEach(async (item) => {
          const canvasId = item.querySelector("div.show_account span.id")?.textContent;
          if (canvasId) {
            const accountLinks = item.querySelector("div.show_account span.links");
            if (isCanvasAccountIdShown) {
              accountLinks.insertAdjacentHTML("afterend", `<div style='color: var(--ic-brand-font-color-dark-lightened-15); font-size: 12px; font-size: 0.75rem;' class='ski-canvas-account-id'>Canvas Account ID: ${canvasId}</div>`);
            }
            if (isSisAccountIdShown) {
